...
def main():
    ...
    for i in exclude_options:
        if i in curl_args:
            quit(
                yellow('Error: {} is not allowed in extra curl args'.format(
                    i)), 1)
    ...
    cmd_core = [
        curl_bin, '-w', curl_format, '-D', headerf.name, '-o', bodyf.name,
        '-s', '-S'
    ]
    ...
    try:
        ...
        print('curl result:', p.returncode, grayscale[16](out),
              grayscale[16](err))
        quit(None, 1)
    for k in d:
    ...
    if show_speed:
        print('speed_download: {:.1f} KiB/s, speed_upload: {:.1f} KiB/s'.
              format(d['speed_download'] / 1024, d['speed_upload'] / 1024))
